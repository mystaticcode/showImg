<template v-cloak>
    <div class="tranvel_list" >
		...
		<ul class="swiper-wrapper">
			<li v-for="item in menus" :key="item.type" class="swiper-slide" :class="{'defaults' : menus_type == item.type}" @click="getName(item.type)">{{item.typeName}}</li>
		</ul>
	</div>
	    ...
	    <ul >
	      <li v-for="item in menus_list" :key="item.dataDetail.id">
	        ...
	        <aside>
	          ...
	          <h4>
	            {{ item.dataDetail.productType }}·{{ item.dataDetail.productCat }}·{{ item.dataDetail.days }}
	          </h4>
			  	<h5>
	            ￥{{ item.dataDetail.priceLabel }}起<em>{{ item.dataDetail.numLabel }}</em>
	          </h5>
	        </aside>
	      </li>
	    </ul>
    ...
    data() {
        ...
        methods:{
            ...
            fiexMenus() {
				...
				sTop >= 1800 ? (this.show = true) : (this.show = false);
			},
			//滑动
			MenusSwiper() {
...
<style scoped>
	...
	.defaults{
		width: 0.05rem;
		background: url(../../../../public/bg/co.png);
